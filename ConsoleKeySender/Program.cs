...
namespace ConsoleKeySender
{
    class Program
    {
        ...
        {
            uint handle, popupHandle;

            //핸들을 찾는다. Spy+를 통해 찾은 클래스 이름과 캡션을 이용하면 된다. 둘 중 하나만 알경우에도 찾을 수 있다. 그때는 하나의 인자를 null로 넘겨 주면된다.
            uint parentHandle = FindWindow(null, "닷넷 C# 개발자 모임");

            //찾은 핸들에서 자식 윈도우 핸들을 찾기 위해서는 FindWindowEx를 이용한다.
            uint windowHandle = FindWindowEx(parentHandle, 0, "EVA_VH_ListControl_Dblclk", null); //화면창

            //PostMessage(handle, EM_SETSEL, 0, uint.MaxValue); //이건 안됨
            //PostMessage(handle, WM_RBUTTONDOWN, 0, 0x003C0005); //마우스 오른클릭

            PostMessage(windowHandle, WM_LBUTTONDOWN, 0, 0x30006); //빈칸 클릭
            PostMessage(windowHandle, WM_LBUTTONUP, 0, 0x30006); //빈칸 클릭
            System.Threading.Thread.Sleep(100);

            PostMessage(windowHandle, WM_RBUTTONUP, 0, 0x39000A); //오른클릭
            System.Threading.Thread.Sleep(100);

            popupHandle = FindWindow("EVA_Menu", null); //화면창
            PostMessage(popupHandle, WM_LBUTTONUP, 0, 0x120031); //전체선택 클릭
            System.Threading.Thread.Sleep(100);
            ...
            popupHandle = FindWindow("EVA_Menu", null); //화면창
            PostMessage(popupHandle, WM_LBUTTONUP, 0, 0x120031); //복사 클릭
            System.Threading.Thread.Sleep(100);
            ...
            //PostMessage(handle, 0x07E9, 0x65, 0);
            //PostMessage(handle, 0x07E9, 0x64, 0);
            //PostMessage(windowHandle, WM_COPY, 0, 0);
            ...
            for (; ; )
            {
                ...
                var s = Clipboard.GetText().TrimEnd();
                var text = s.Substring(s.LastIndexOf('\n'));

                //Console.WriteLine(text);

                if (text.IndexOf("bluepope") > -1)
                {
                    handle = FindWindowEx(parentHandle, 0, "RichEdit20W", null);

                    var nickName = text.Substring(2, text.IndexOf(']')-2);

                    if (nickName.ToLower() != "bluepope")
                    {
                        //복사가 좀 늦게 클릭되면 이부분에서 문제가 발생함
                        Clipboard.SetText($"{nickName}님 잠시만 기다려주세요");
                        System.Threading.Thread.Sleep(100);
                        //Clipboard.SetText($"{nickName}님 잠시만 기다려주세요");
